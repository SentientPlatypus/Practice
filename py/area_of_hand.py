...
class AreaCalculator:
    def __init__(self, hand_image:np.ndarray, area_of_paper:float) -> None:
        self.hand_image = cv2.resize(hand_image, )
    ...
    def calculate_area(self) -> float:
        ...
        cv2.putText(self.hand_image, f"Paper Area: {paper_area:.2f} pixels", (x_paper, y_paper), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
        cv2.putText(self.hand_image, f"Hand Area: {hand_area:.2f} pixels", (x_hand, y_hand), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)

        print(f"Paper Area: {paper_area:.2f}, hand area: {hand_area:.2f}")
...
if __name__ == "__main__":
    handimage = cv2.imread("genedarkhand.JPG")
    areacalculator = AreaCalculator(handimage, area_of_paper= 21.59 * 28)
